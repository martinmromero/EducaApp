...
(out / 'test_single_export.pdf').write_bytes(render_exam_payload_to_pdf(payload, formato))
(out / 'test_single_export.docx').write_bytes(render_exam_payload_to_docx(payload, formato))
...
if batch:
    ...
    (out / 'test_batch_export.pdf').write_bytes(render_exam_batch_payloads_to_pdf(docs))
    (out / 'test_batch_export.docx').write_bytes(render_exam_batch_payloads_to_docx(docs))

print('OK', exam.id, bool(batch))
